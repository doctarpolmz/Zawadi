...
export default async function BrowseMusicPage({ searchParams }: PageProps) {
  ...
  const tracks = (await query.limit(48) as any).data as any[]
  const { data: categories } = await supabase.from('categories').select('*').eq('type', 'music')

  return (
    <div className="px-4 md:px-8 max-w-screen-2xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white">🎵 Music</h1>
        <p className="text-zinc-400 mt-1">Afrobeats, Bongo Flava, Highlife and more</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {tracks?.map((track) => <ContentCard key={track.id} content={track as any} />)}
      </div>
    </div>
  )
}
